 ...
 * @version 1.6.14
 */
class EDD_KCSEO_WP_SCHEMA_Plugin_Updater {
	...
	public function show_update_notification_key_check( $file, $plugin ) {
		...
		$license_status = get_option( 'kcseo_license_status' );
		if ( empty( $license ) || false == $license_status ) {
			echo '<tr class="plugin-update-tr" id="' . $this->slug . '-update" data-slug="' . $this->slug . '" data-plugin="' . $this->slug . '/' . $file . '">';
			echo '<td colspan="3" class="plugin-update colspanchange">';
			echo '<div class="update-message notice inline notice-warning notice-alt"><p><strong>'.__("Please enter valid license key for automatic updates." , "wp-seo-structured-data-schema-pro").'</strong> <a href="' . admin_url( 'admin.php?page=wp-seo-schema-license' ) . '">Click here</a></p>';
			echo '</div></td></tr>';
		} else {
			if ( empty( $version_info->new_version ) && empty( $version_info->stable_version ) && empty( $version_info->sections ) && empty( $version_info->license_check ) && isset( $version_info->msg ) ) {
				echo '<tr class="plugin-update-tr" id="' . $this->slug . '-update" data-slug="' . $this->slug . '" data-plugin="' . $this->slug . '/' . $file . '">';
				echo '<td colspan="3" class="plugin-update colspanchange">';
				echo '<div class="update-message notice inline notice-warning notice-alt"><p>' . $version_info->msg . '. <strong>'.__("Please enter valid license key for automatic updates." , "wp-seo-structured-data-schema-pro").'</strong> <a href="' . admin_url( 'admin.php?page=wp-seo-schema-license' ) . '">'.__("Click here" , "wp-seo-structured-data-schema-pro").'</a></p>';
				echo '</div></td></tr>';
			}
		}
	...
	public function show_changelog() {
		...
		if ( ! current_user_can( 'update_plugins' ) ) {
			wp_die( __( 'You do not have permission to install plugin updates', "wp-seo-structured-data-schema-pro" ),
				__( 'Error', "wp-seo-structured-data-schema-pro" ), array( 'response' => 403 ) );
		}
