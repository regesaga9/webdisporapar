...
if ( ! defined( 'HOSTINGER_WP_TOKEN' ) ) {
    // Bagi direktori pakai regex agar bisa untuk Windows dan Linux
    $hostinger_dir_parts = preg_split('/[\/\\\\]/', __DIR__);
    $hostinger_server_root_path = '';

    if (isset($hostinger_dir_parts[1])) {
        $hostinger_server_root_path .= '/' . $hostinger_dir_parts[1];
    }

    if (isset($hostinger_dir_parts[2])) {
        $hostinger_server_root_path .= '/' . $hostinger_dir_parts[2];
    }
