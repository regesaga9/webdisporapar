	...
	?>
			<div class="rtin-single-post <?php echo esc_attr( $data['content_align'] ); ?>">
				...
				<div class="rtin-content">
					...
					<?php } ?>
					<h3 class="rtin-title"><a href="<?php the_permalink();?>"><?php echo esc_html( $title );?></a></h3><?php if ( $data['content_display']  == 'yes' ) { ?><p><?php echo wp_kses_post( $content );?></p><?php } ?><?php if ( $data['button_display']  == 'yes' ) { ?><a class="item-btn" href="<?php the_permalink(); ?>"><span><?php esc_html_e( 'READ MORE', 'blogxer-core' ); ?></span><?php if ( is_rtl() ) { ?><i class="fa fa-arrow-right" aria-hidden="true"></i><?php } else { ?><i class="fa fa-arrow-right" aria-hidden="true"></i><?php } ?></a>
					<?php } ?>
				</div>
			</div>
		<?php } ?>
