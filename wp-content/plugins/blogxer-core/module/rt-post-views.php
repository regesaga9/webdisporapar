...
if( ! class_exists( 'RT_POST_VIEWS' )){
	...
	class RT_POST_VIEWS{
		...
		function _set_post_views(){
			...
			if( !BlogxerTheme::$options['post_view'] || ! is_single() ){
				return;
			}
			...
			if( !BlogxerTheme::$options['post_view'] || $page > 1  ){
				return false;
			}
		...
		function _posts_column_views( $defaults ){
			...
			if( BlogxerTheme::$options['post_view'] ){
				$defaults['post_view'] = esc_html__( 'Views', 'blogxer-core' );
			}
...
/*----------------------------------------------------------*/
if( !function_exists( 'blogxer_views' )){

	function blogxer_views( $text = '', $post_id = 0 ){
		...
		if ( $view_count == 1 ) {
			$blogxer_view_html = esc_html__( 'View' , 'blogxer-core' );
		} else {
			$blogxer_view_html = esc_html__( 'Views' , 'blogxer-core' );
		}
		
		$blogxer_views_html = $view_count . ' ' . $blogxer_view_html;

		return '<span class="meta-views meta-item '. $views_class .'">'. $blogxer_views_html.'</span> ';
	}
}
