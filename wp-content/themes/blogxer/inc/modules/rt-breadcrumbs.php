...
if( ! function_exists( 'blogxer_breadcrumbs' )){

	function blogxer_breadcrumbs(){
			
		if( BlogxerTheme::$options['breadcrumb_active'] == '0' || BlogxerTheme::$options['breadcrumb_hide_mobile'] == '0' ) {			
			return;
		}

		# breadcrumbs ----------
		$delimiter  = BlogxerTheme::$options['breadcrumbs_delimiter'] ? wp_kses_post( BlogxerTheme::$options['breadcrumbs_delimiter'] ) : '&#47;';
		$delimiter  = '<em class="delimiter">'. $delimiter .'</em>';

		$home_text  = esc_html__( 'Home', 'blogxer' );
		...
		if ( ! is_home() && ! is_front_page() || is_paged() ){
			...
			elseif ( is_singular() ){			
				...
				if ( get_post_type() == 'post' ){
					...
					$category = blogxer_get_primary_category_id();
					$category = get_category( $category );
